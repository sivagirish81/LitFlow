        ...
        {corpusId && (
          <nav className="mt-3 flex flex-wrap gap-2 rounded-2xl border border-black/10 bg-white/80 p-2">
            <Link
              ...
              className={`rounded-xl px-4 py-2 text-sm font-medium ${current === "search" ? "bg-ink text-white" : "text-zinc-700 hover:bg-black/5"}`}
            >
              Semantic Search
            </Link>
            <Link
              href={`/corpora/${corpusId}/survey`}
              className={`rounded-xl px-4 py-2 text-sm font-medium ${current === "survey" ? "bg-ink text-white" : "text-zinc-700 hover:bg-black/5"}`}
            >
              Survey Builder
            </Link>
            <Link
              href={`/corpora/${corpusId}/graph`}
              className={`rounded-xl px-4 py-2 text-sm font-medium ${current === "graph" ? "bg-ink text-white" : "text-zinc-700 hover:bg-black/5"}`}
            >
              Knowledge Graph
            </Link>
          </nav>
